...
export function* dummyDataSagaFlow() {
  try {
      const response = yield call(API.apiDummyDataRequest);
      yield put(ACTIONS.getDummyDataReceive(response));
    } catch (err) {
...
export function* saga1() {
  ...
  yield takeLatest(TYPES.GET_DUMMY_DATA_REQUEST, dummyDataSagaFlow);
}
